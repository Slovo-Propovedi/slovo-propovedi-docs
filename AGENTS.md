...
6. **Технический долг: новую задолженность исправляем сразу, а не записываем.** Когда долг закрыт — **удаляй** его запись из `docs/debt.md`: файл перечисляет только **открытые** долги, закрытые записи удаляются, а не архивируются.
