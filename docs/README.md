...
Проект: автономный Swagger UI + OpenAPI-спецификация для **Admin API — Слово.Проповеди** — полностью статический nginx-контейнер, не зависящий от админ-бэкенда (нет Postgres, MinIO или контейнеров бэкенда). Стек, архитектура и команды — в [`AGENTS.md`](../AGENTS.md).
