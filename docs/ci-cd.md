...
CI использует `actions/setup-node@v4` с `node-version: 20` — **это совпадает со Stage 1 Dockerfile** (`node:20-alpine`). (Для сравнения: в мобильном проекте — Node 24; здесь — 20, смотри на реальные файлы.) У проекта нет своих серверных зависимостей, Node нужен только чтобы запустить `validate:openapi` и `npm ci`.
